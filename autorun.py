...
# nodes = [500, 1500, 2500, 3500, 4500, 5500, 6500]
nodes = [500]
...
def write_config(template):
    with open(simconf_path, "w") as fp:
        fp.write(template)
    print("write success")


def launch_simulator(param_str):
    args = ["bash", "autorun.sh", param_str]
    ret = subprocess.run(args=args, capture_output=True, text=True)

    # 標準出力を取得する
    output_text = ret.stdout
...
def get_result(root_directory, prefix):
    ...
    with open(target_summary) as fp:
