 ...
 */
 ...
 public class BitcoinCoreTable extends AbstractRoutingTable {
   ...
   public ArrayList<Integer> proposalMethod(int selectNum) {
    // 遅延時間を元にして選択する地域を選ぶ
     ...
     for (int i = 0; i < interregional_delay.length; i++) {
       ...
       // n乗根
       total_outbound_latency += Math.pow(x, 1.0/n);
     }
     ...
     for (int i = 0; i < interregional_delay.length; i++) {
       ...
       double x = (latency + propagationtime);
       // n乗根
       double target_x = Math.pow(x, 1.0/n);
       //確率
   ...
   public void initTable() {
     ...
     int foregin_region_num = 1;
     // 提案手法でリージョンを選ぶ
     ArrayList<Integer> selectedIndexes = proposalMethod(foregin_region_num);
     // 既存手法でリージョンを選ぶ
