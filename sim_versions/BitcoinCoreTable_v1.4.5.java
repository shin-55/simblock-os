 ...
 */
 ...
 public class BitcoinCoreTable extends AbstractRoutingTable {
   ...
   public ArrayList<Integer> proposalMethod(ArrayList<Integer> candidates) {
    
     Node selfNode = getSelfNode();
     // 遅延時間を元にしてOutBoundに選択する地域を選ぶ
     int selfRegion = selfNode.getRegion();
     long[] node_latency = LATENCY[selfRegion];
     ...
     for (int i = 0; i < node_latency.length; i++) {
       // 自分の地域は除く
       if(i == selfRegion){
         continue;
       }
       // 累積遅延時間へ加算
       long latency = node_latency[i];
       total_outbound_latency += latency;
     }
     ...
     for (int i = 0; i < node_latency.length; i++) {
       ...
       //確率を計算
       long latency = node_latency[i];
       double x = (double)latency / total_outbound_latency;

       // n乗根
       double n = 2.0;
       double ret = Math.pow(x, 1.0/n);
   ...
   public void initTable() {
     ...
     ArrayList<Integer> selectedIndexes = proposalMethod(candidates);
   ...
   public boolean addNeighbor(Node node) {
     ...
     // inbound関係の処理
     boolean inboundResult = addNeighborInbound(selfNode);
